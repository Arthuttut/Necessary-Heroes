PalladiumEvents.registerAnimations((event) => {
    event.register('nc:prime_prototype', 10, (builder) => {
        const progress = animationUtil.getAnimationTimerAbilityValue(builder.getPlayer(), 'nc:ben10/prototype/prototype', 'prime_animation', builder.getPartialTicks());
        ...
        if (abilityUtil.isEnabled(builder.getPlayer(), 'nc:ben10/prototype/prototype', 'dial_cycle_animation')) {
            if (!builder.isFirstPerson()) {
                builder.get('right_arm').setYRotDegrees(-20)
            }
            else {
                builder.get('right_arm').setYRotDegrees(-20)
            }
